...
app.use(express.static('public')); // public 폴더의 html 서빙

// Gemini API 설정
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);

// [데이터] 연령별 맞춤 시나리오 (전문용어 배제 및 수준 조절)
const SCENARIOS = {
    elementary: {
        target: "초등학생",
    ...
    high: { // 고등학생, 대학생, 성인은 원본 시나리오 기반 (단, 용어는 대상에 맞춰 미세 조정 가능하나 여기선 원본 유지)
        target: "고등학생/대학생/성인",
...
app.post('/api/evaluate', async (req, res) => {
    ...
    try {
        ...
        학습자(${scenarioData.target})가 작성한 [학교 급식 '플라스틱 제로' 전환 프로젝트] 계획서를 평가해야 합니다.

        [평가 원칙]
        1. 절대 무조건 칭찬하지 마십시오. 냉철하게 분석하세요.
        2. 아래 '8가지 핵심 역량 평가 기준'을 엄격히 적용하여 점수를 매기세요.
        3. 학습자의 연령(${scenarioData.target})을 고려하여 피드백의 말투는 이해하기 쉽게 하되, 평가 기준(점수) 자체는 타협하지 마십시오.
        
        [8가지 핵심 역량 평가 기준 (Rubric)]
        1. 시스템사고: 환경-경제-사회 3가지 영역의 순환적 인과관계나 상충 관계(Trade-off) 설명 여부 (단순 나열은 감점)
        2. 미래사고: 5년 후 시나리오의 구체성 및 '의도하지 않은 부작용'에 대한 사전 예방책 유무
        3. 가치사고: '정의로운 전환', '형평성' 등 윤리적 기준 적용 및 명확한 타협 기준 제시
        4. 전략적 사고: 전략의 실현 가능성과 장애물(예산, 반대)에 대한 실질적 극복 방안
        5. 실천 역량: 누가(Who), 언제(When), 어떻게(How)가 명확하고 주도적인지
        6. 대인관계 역량: 갈등 관계인 이해관계자(조리 실무자 등)를 파트너로 포용하고 공감하는지
        7. 자기이해 역량: 자신의 한계(메타인지)를 알고 구체적인 회복 탄력성/스트레스 관리 방안이 있는지
        8. 통합적 문제해결력: 위 7가지가 논리적 모순 없이 하나로 연결되는지

        [학습자가 보고 작성한 시나리오]
        배경: ${scenarioData.background}
        문제: ${scenarioData.problems.join(', ')}
        
        [학습자의 답변]
        ${userResponse}

        ---------------------------------------------------------
        [출력 형식] (Markdown 형식 필수)

        ## 📊 [지속가능성 핵심역량 진단 성적표]

        | 핵심 역량 | 점수 (5점 만점) | 상세 평가 및 피드백 |
        | :--- | :---: | :--- |
        | **시스템사고** | ⭐⭐⭐ (3/5) | (평가 내용) |
        | **미래사고** | ... | ... |
        ... (8개 항목 모두 작성) ...
        ...
        ## 💡 총평 및 코칭
        * **강점:** (높은 점수 항목 칭찬)
        * **보완점:** (낮은 점수 항목에 대해 '상' 등급을 받기 위한 구체적이고 실천적인 조언. ${scenarioData.target} 눈높이에 맞춰 설명)
        `;
